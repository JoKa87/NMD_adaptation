...
params = {
         ...
         "expression_path"      : parent_dir+r"\data\tcga_avg_expressions.txt",
         "feature_targets"      : [],
         ...
         "processed_fname"      : "msk_chord_zeros_included_filtered.txt",
...
def main():
    ...
    if params["mode"] == "append_treatment_by_time" or params["mode"] == "append_treatment_by_time2":
        ...
        # load mutation data
        mutations = pd.read_csv(params["data_dir"]+params["os_sep"]+mutations_fname, sep=",")
        ...
        treatment_starts = append_df_with_mapping([mutations, immune_treatment], patient_col, "PATIENT_ID", "START_DATE",
                                                  set_bar("mapping treatment starts"), non_redundant=True, reverse=False)
        ...
        # marked (<-) added on 250527 to include treatment times
        treatment_starts = [int(treatment_start) if treatment_start != "-" else None for treatment_start in treatment_starts]
        if "ID:subtype" not in mutations.columns: mutations.insert(mutations.shape[1], "ID:treatment start", treatment_starts)
        else:                                     mutations["ID:treatment start"] = treatment_starts
        ...
        mutations = pmu.append_markers(mutations, patient_col)
        mutations.to_csv(params["data_dir"]+params["os_sep"]+mutations_fname.split(".")[0]+"_appended.txt", sep=",", index=False)
        pmu.test_ptcs(mutations, specimen, treatment, patient_col, sample_col)
    ...
    if params["mode"] == "filter_mutations":
        ...
        # print to file
        mutations.to_csv(params["data_dir"]+params["os_sep"]+params["fnames"]["ptcs"].split(".")[0]+"_filtered_samples.txt", sep=",", index=False)
    ...
    if params["mode"] == "prepare_analysis" or params["mode"] == "test_patient_data":
        ...
        ptcs["expression"] = [float(expression) if expression != "-" else median_expression for expression in expressions]


        # prepare analysis
        if params["mode"] == "prepare_analysis":
            processed_patient_data = pmu.prepare_analysis(sample_data, ptcs)
